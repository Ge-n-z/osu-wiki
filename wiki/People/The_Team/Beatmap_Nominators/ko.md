...
비트맵 노미네이터의 멤버들은 주로 [펜딩 비트맵](/wiki/Beatmap/Category#work-in-progress-and-pending)의 [모딩](/wiki/Modding) 및 랭킹 사이클에 올리는 것을 담당합니다. 비트맵 노미네이터는 매핑 또는 모딩과 관련된 조언을 합니다. 지명된 비트맵이 직접적으로 랭킹 기준을 위반하지 않더라도 수정해야 하는 문제가 있다고 생각되면 이들은 [비트맵 거부권](Beatmap_Veto)을 사용할 수 있습니다.

추가로, 모든 비트맵 노미네이터들은 [비트맵 노미네이터에 대한 기대](Expectations)에 나온 대로 비트맵과 매퍼의 더 폭 넓은 관심을 유도하며 쉬지 않고 실력 향상을 위해 노력함으로써 더욱 큰 커뮤니티에 초점을 맞추는 것을 고수할 것으로 예상됩니다.
...
아래 나열된 표는 각 비트맵 노미네이터가 공식적으로 지명할 수 있는 게임 모드를 나타냅니다. *참고: 모든 비트맵 노미네이터들은 아래 추가로 명시된 언어 이외에는 영어를 사용합니다.*
...
- 2015년 2월 1일, *비트맵 감사팀*에서 *비트맵 노미네이션 그룹*으로 변경되었고, 후에 우리가 아는 비트맵 노미네이터로 바뀌게 됩니다.
